...
namespace CMS.Common
{
    public class Utils
    {
        ...
        private static Dictionary<int, string> SKUYearDictionary = new() { { 2021, "A" }, { 2022, "B" }, { 2023, "C" }, { 2024, "D" } };
        ...
        /// <summary>
        /// Check Content Has Base 64 in Editor
        /// </summary>
        /// <param name="content"></param>
        /// <returns></returns>
        public static bool CheckContentHasBase64(string content)
        {
        ...
        /// <param name="content"></param>
        /// <returns></returns>
        public static string UploadImgBase64Content(string imgName, string rootPath, string pathSave, string content)
        {
            var regex = new Regex(@"<img src=""(?<data>.*)""");
            var match = regex.Matches(content).ToList();
            foreach (var file in match)
            {
                if (file.Groups["data"].Value.StartsWith("data:image"))
                {
                    ...
                    var physicalPath = Path.Combine(rootPath, pathSave);
        ...
        public static string SKUGetByYear(int year)
        {
            return SKUYearDictionary.GetValueOrDefault(year);
        }
    }
}