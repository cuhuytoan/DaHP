namespace CMS.Services.RepositoriesBase
{
    public abstract class RepositoryBase<T> : IRepositoryBase<T> where T : class
    {
        ...
        public async ValueTask<T> FindAsync(object id)
        {
            return await CmsContext.Set<T>().FindAsync(id);
        }
        ...
        {
            return await CmsContext.Set<T>().LastOrDefaultAsync(expression);
        }
        ...
        public async Task SendMail(string FromName, string ToEmail, string ToName, string Subject, string Body)
        {
            ...
            MailAddress FromAddress = new(setting.EmailSender, FromName);
            MailAddress ToAddress = new(ToEmail, ToName);
            MailMessage Mailer = new(FromAddress, ToAddress);
        ...
        {
            return await CmsContext.Set<T>().CountAsync();
        }
